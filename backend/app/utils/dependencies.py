...
# app/utils/dependencies.py - Add debug logging to get_current_user
...
async def get_current_employer(
    ...
    if current_user.role != "EMPLOYER":
        print(f"❌ Access denied: User role is '{current_user.role}', expected 'EMPLOYER'")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Employer access required. Only employers can create job postings."
        )
